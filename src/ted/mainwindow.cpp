...
bool MainWindow::isValidMonkeyPath( const QString &path ){
    QString trans="trans"+HOST;
#ifdef Q_OS_WIN
    trans+=".exe";
#endif
    return QFile::exists( path+"/bin/"+trans );
}
...
void MainWindow::enumTargets(){
    ...
    QString cmd="\""+_monkeyPath+"/bin/trans"+HOST+"\"";
    ...
    while( proc.waitLineAvailable( 0 ) ){
        ...
        if( line.startsWith( sol ) ){
            ...
            for( int i=0;i<bits.count();++i ){
                QString bit=bits[i];
                if( !bit.isEmpty() ) _targetsWidget->addItem( bit );
            }
        }
    }
}
...
void MainWindow::readSettings(){
    ...
    _buildMonkeyCmd="\"${MONKEYPATH}/bin/trans"+HOST+"\" -target=${TARGET} -config=${CONFIG} \"${FILEPATH}\"";
    _runMonkeyCmd="\"${MONKEYPATH}/bin/trans"+HOST+"\" -target=${TARGET} -config=${CONFIG} -run \"${FILEPATH}\"";
...
void MainWindow::runCommand( QString cmd,QWidget *fileWidget ){
    ...
    cmd=cmd.replace( "${TARGET}",_targetsWidget->currentText() );
