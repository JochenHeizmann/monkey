...
class bb_opengl_gles20{
	...
	static void _glBufferData( int target,int size,DataBuffer data,int usage ){
		GLES20.glBufferData( target,size,data.GetBuffer(),usage );
	}
	
	static void _glBufferSubData( int target,int offset,int size,DataBuffer data ){
		GLES20.glBufferSubData( target,offset,size,data.GetBuffer() );
	}
	...
	static void _glReadPixels( int x,int y,int width,int height,int format,int type,DataBuffer pixels ){
		GLES20.glReadPixels( x,y,width,height,format,type,pixels.GetBuffer() );
	}

	static void _glTexImage2D( int target,int level,int internalformat,int format,int type,String path ){
		int[] info={0,0};
		DataBuffer pixels=LoadImageData( path,info );
		if( pixels!=null ){
			GLES20.glTexImage2D( target,level,internalformat,info[0],info[1],0,format,type,pixels.GetBuffer() );
		}
	}
	...
	static void _glTexImage2D( int target,int level,int internalformat,int width,int height,int border,int format,int type,DataBuffer pixels ){
		GLES20.glTexImage2D( target,level,internalformat,width,height,border,format,type,pixels.GetBuffer() );
	}

	static void _glTexSubImage2D( int target,int level,int xoffset,int yoffset,int format,int type,String path ){
		int[] info={0,0};
		DataBuffer pixels=LoadImageData( path,info );
		if( pixels!=null ){
			GLES20.glTexSubImage2D( target,level,xoffset,yoffset,info[0],info[1],format,type,pixels.GetBuffer() );
		}
	}
	...
	static void _glTexSubImage2D( int target,int level,int xoffset,int yoffset,int width,int height,int format,int type,DataBuffer pixels ){
		GLES20.glTexSubImage2D( target,level,xoffset,yoffset,width,height,format,type,pixels.GetBuffer() );
	}
