...
void _glTexImage2D( int target,int level,int internalformat,int format,int type,String path ){
	Array<int> info(2);
	DataBuffer *pixels=LoadImageData( path,info );
	if( pixels ){
		glTexImage2D( target,level,internalformat,info[0],info[1],0,format,type,pixels->ReadPointer() );
	}
}
...
void _glTexSubImage2D( int target,int level,int xoffset,int yoffset,int format,int type,String path ){
	Array<int> info(2);
	DataBuffer *pixels=LoadImageData( path,info );
	if( pixels ){
		glTexSubImage2D( target,level,xoffset,yoffset,info[0],info[1],format,type,pixels->ReadPointer() );
	}
}
...
void _glVertexAttribPointer( int indx, int size, int type, bool normalized, int stride, DataBuffer *ptr ){
